<template>
	<view>
		...
		<!-- 模态框 -->
		<popup-spec @add="add" @sub="sub" @hideSpec="hideSpec" @setSelectSpec="setSelectSpec" :spaceInfo="spaceInfo"  :goodsInfo="goodsInfo"  :goodsData="goodsData"></popup-spec>  <!-- 属性传值 -->
	...
	export default {
		...
		data(){
			return {
				isKeep:false, //收藏
				currentSwiper:0, //下标值
				spaceInfo:{showSpace:false}, //点击显示模态框 默认值为false
				goodsData:{
					swiperList:[],
					spec:[],
					comment:[{
					...
					success:(res =>{
						...
						goodsList.forEach(goods => {
							...
							if(goods.goods_id == this.goodsInfo.goods_id && goods.spec == this.goodsInfo.spec){
								...
								isExist = true; //同款规格的商品添加两次 就不需要重新添加本地存储中
							}
						})
						if(!isExist){ 
							goodsList.push(this.goodsInfo);
							//如果商品不存在就直接加入存储 更新本地存储
