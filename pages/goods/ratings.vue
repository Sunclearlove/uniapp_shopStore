<template>
	<view>
		<video v-show="showVideo" id="myVideo" :show-fullscreen-btn="showFullscreenBtn" :direction="videoDirection" 
				@pause="videoPause" @fullscreenchange="viderFullscreen" class="myVideo" :src="videoSrc"></video>
	<view class="content">
		<view class="label">
			<view @tap="labelSelect(index)" :class="{'on': index == labelIndex}" v-for="(label,index) in labelList" :key="index">
				{{label.name}}({{label.number}})
			</view>
		</view>
					<view class="list">
						<view class="row" v-for="(row,index) in ratingsList" :key="index">
							<view class="left">
								<view class="face">
							...
							<view class="right">
								...
								<view class="first">
									...
									<view class="img-video">
										<view @tap="playVideo(item.path)" class="box" v-for="item in row.video" :key="item.path">
...
<script>
	export default {
		data(){
			return{
				showVideo:true, //判断进入小程序
				ratingsList:[],
				videoSrc:"",
				labelIndex:0, //定义默认下标值0为选中
